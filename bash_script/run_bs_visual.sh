...
SAVE_DIR="$PROJECT_ROOT/saved_model/bs/visual"
...
run_dataset_experiments() {
    ...
    echo "Starting VCRKT Visual experiments for Dataset: $DATASET on GPU: $GPU_ID"
    
    # 遍历 5 折 (串行)
    for FOLD in 0 1 2 3 4; do
        LOG_FILE="$LOG_DIR/visual_${DATASET}_fold${FOLD}.log"
        
        echo "[GPU $GPU_ID] Running: Dataset=$DATASET Type=Visual Fold=$FOLD"
        ...
        echo "[GPU $GPU_ID] Finished: Dataset=$DATASET Type=Visual Fold=$FOLD"
    done
}

# 并行运行三个数据集 (GPU分配: 1, 2, 3)
