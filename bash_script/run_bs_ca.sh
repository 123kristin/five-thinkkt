...
SAVE_DIR="$PROJECT_ROOT/saved_model/bs/ca"
...
run_dataset_experiments() {
    ...
    echo "Starting VCRKT Cross-Attention (CA) experiments for Dataset: $DATASET on GPU: $GPU_ID"
    
    # 遍历 5 折 (串行)
    for FOLD in 0 1 2 3 4; do
        LOG_FILE="$LOG_DIR/ca_${DATASET}_fold${FOLD}.log"
        
        echo "[GPU $GPU_ID] Running: Dataset=$DATASET Type=CA Fold=$FOLD"
        ...
        echo "[GPU $GPU_ID] Finished: Dataset=$DATASET Type=CA Fold=$FOLD"
    done
}
...
# 等待所有后台任务完成
echo "All CA experiments launched in parallel. Waiting for completion..."
wait
echo "All CA experiments finished."
