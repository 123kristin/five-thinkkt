        ...
        if torch.cuda.is_available():
            ...
            try:
                ...
                if gpu_id < torch.cuda.device_count():
                    ...
                    print(f"[CRKTNet] 使用指定GPU: cuda:{gpu_id}")
                else:
                    self.device = torch.device("cuda:0")
                    print(f"[CRKTNet] 指定GPU {gpu_id} 不可用，使用默认GPU: cuda:0")
            except ValueError:
                self.device = torch.device("cuda:0")
                print(f"[CRKTNet] GPU ID解析失败，使用默认GPU: cuda:0")
        else:
            self.device = torch.device("cpu")
            print(f"[CRKTNet] CUDA不可用，使用CPU")
    ...
    def __init__(self, config):
        ...
        if torch.cuda.is_available():
            ...
            try:
                ...
                if gpu_id < torch.cuda.device_count():
                    self.device = torch.device(f"cuda:{gpu_id}")
                    print(f"[CRKT] 使用指定GPU: cuda:{gpu_id}")
                else:
                    self.device = torch.device("cuda:0")
                    print(f"[CRKT] 指定GPU {gpu_id} 不可用，使用默认GPU: cuda:0")
            except ValueError:
                self.device = torch.device("cuda:0")
                print(f"[CRKT] GPU ID解析失败，使用默认GPU: cuda:0")
        else:
            self.device = torch.device("cpu")
            print(f"[CRKT] CUDA不可用，使用CPU")
