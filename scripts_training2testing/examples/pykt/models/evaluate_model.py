...
def save_cur_predict_result(dres, q, r, d, t, m, sm, p):
    ...
    # 为避免设备不一致导致的masked_select错误，这里统一移到CPU后再处理
    ...
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            if model_name in ["dimkt"]:
                ...
                sd, qd, sdshft, qdshft = sd.to(next(model.parameters()).device), qd.to(next(model.parameters()).device), sdshft.to(next(model.parameters()).device), qdshft.to(next(model.parameters()).device)
            else:
                q, c, r = dcur["qseqs"], dcur["cseqs"], dcur["rseqs"]
                qshft, cshft, rshft = dcur["shft_qseqs"], dcur["shft_cseqs"], dcur["shft_rseqs"]
            m, sm = dcur["masks"], dcur["smasks"]
            q, c, r, qshft, cshft, rshft, m, sm = q.to(next(model.parameters()).device), c.to(next(model.parameters()).device), r.to(next(model.parameters()).device), qshft.to(next(model.parameters()).device), cshft.to(
                next(model.parameters()).device), rshft.to(next(model.parameters()).device), m.to(next(model.parameters()).device), sm.to(next(model.parameters()).device)
            ...
            # 统一到同一设备，避免CUDA/CPU不一致
            model_device = next(model.parameters()).device
            sm = sm.to(model_device)
            y = y.to(model_device)
            y = torch.masked_select(y, sm).detach().cpu()
            # print(f"pred_results:{y}")  
            rshft = rshft.to(model_device)
...
def early_fusion(curhs, model, model_name):
    if model_name in ["dkvmn", "skvmn"]:
        ...
        p = p.squeeze(-1)
    elif model_name in ["akt", "extrakt", "folibikt", "dtransformer", "simplekt", "stablekt", "cskt", "fluckt",
                        "bakt_time", "sparsekt", "lefokt_akt", "ukt", "akt_vector", "akt_norasch", "akt_mono",
                        "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy", "aktvec_raschx"]:
...
def effective_fusion(df, model, model_name, fusion_type):
    ...
    hasearly = ["dkvmn", "deep_irt", "skvmn", "kqn", "akt", "extrakt", "folibikt", "dtransformer", "simplekt",
                "stablekt", "cskt", "fluckt", "ukt", "bakt_time", "sparsekt", "lefokt_akt", "saint", "sakt", "hawkes",
                "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx",
                "akt_raschy", "aktvec_raschx", "lpkt"]
    for ui in df:
        # 一题一题处理
        curdf = ui[1]
    ...
    if "early_fusion" in fusion_type and model_name in hasearly:
        curhs = [torch.tensor(np.array(curh)).float().to(next(model.parameters()).device) for curh in curhs]
        curr = torch.tensor(np.array(curr)).long().to(next(model.parameters()).device)
...
def save_question_res(dres, fout, early=False):
    ...
    for i in range(0, len(dres["qidxs"])):
        ...
        if early:
            ...
            curres = curres + [et, ep]
        curstr = "\t".join(
            [str(round(s, 4)) if type(s) == type(0.1) or type(s) == np.float32 else str(s) for s in curres])
        fout.write(curstr + "\n")
...
def evaluate_question(model, test_loader, model_name, fusion_type=["early_fusion", "late_fusion"], save_path=""):
    ...
    hasearly = ["dkvmn", "deep_irt", "skvmn", "kqn", "dtransformer", "akt", "extrakt", "folibikt", "simplekt", "cskt",
                "fluckt", "stablekt", "ukt", "bakt_time", "sparsekt", "lefokt_akt", "saint", "sakt", "hawkes",
                "akt_vector", "akt_norasch", "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx",
                "akt_raschy", "aktvec_raschx", "lpkt"]
    if save_path != "":
        fout = open(save_path, "w", encoding="utf8")
        if model_name in hasearly:
            fout.write("\t".join(
                ["orirow", "qidx", "questions", "concepts", "concept_preds", "late_trues", "late_mean", "late_vote",
                 "late_all", "early_trues", "early_preds"]) + "\n")
        else:
            fout.write("\t".join(
                ["orirow", "qidx", "questions", "concepts", "concept_preds", "late_trues", "late_mean", "late_vote",
                 "late_all"]) + "\n")
    with torch.no_grad():
        ...
        for data in test_loader:
            ...
            if model_name in ["dkvmn", "skvmn"]:
                ...
                y, h = model(dcurori, qtest=True, train=False)
            elif model_name in ["akt", "extrakt", "folibikt", "fluckt", "lefokt_akt", "akt_vector", "akt_norasch",
                                "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy",
                                "aktvec_raschx"]:
                ...
                start_hemb = torch.tensor([-1] * (h.shape[0] * h.shape[2])).reshape(h.shape[0], 1, h.shape[2]).to(
                    next(model.parameters()).device)  # add the first hidden emb
...
def evaluate_splitpred_question(model, data_config, testf, model_name, save_path="", use_pred=False, train_ratio=0.2,
                                atkt_pad=False):
    if save_path != "":
        fout = open(save_path, "w", encoding="utf8")
    if model_name == "lpkt":
    ...
    with torch.no_grad():
        ...
        for i, row in df.iterrows():
            ...
            if model_name == "lpkt":
                ...
                with open(
                        f'/root/autodl-nas/project/pykt_nips2022/data/{dataset_name}/skills_difficult_{model.difficult_levels}.csv',
                ...
                with open(
                        f'/root/autodl-nas/project/pykt_nips2022/data/{dataset_name}/questions_difficult_{model.difficult_levels}.csv',
                        'r', encoding="UTF8") as f:
                    reader = csv.reader(f)
            ...
            # print(concepts)
            # print(responses)
            cq = torch.tensor([int(s) for s in questions]).to(next(model.parameters()).device)
            cc = torch.tensor([int(s) for s in concepts]).to(next(model.parameters()).device)
            cr = torch.tensor([int(s) for s in responses]).to(next(model.parameters()).device)
            ct = torch.tensor([int(s) for s in times]).to(next(model.parameters()).device)
            dtotal = {"cq": cq, "cc": cc, "cr": cr, "ct": ct}
            if model_name == "lpkt":
                cit = torch.tensor([int(s) for s in it_times]).to(next(model.parameters()).device)
                dtotal["cit"] = cit
            elif model_name == "dimkt":
                # print(f"cq:{cq}")
                # print(f"seq_sds:{seq_sds}")
                csd = torch.tensor(seq_sds).to(next(model.parameters()).device)
                cqd = torch.tensor(seq_qds).to(next(model.parameters()).device)
                dtotal["csd"] = csd
                dtotal["cqd"] = cqd
            ...
            curdforget = dict()
            for key in dforget:
                dforget[key] = torch.tensor(dforget[key]).to(next(model.parameters()).device)
...
def predict_each_group(dtotal, dcur, dforget, curdforget, is_repeat, qidx, uid, idx, model_name, model, t, end, fout,
                       atkt_pad=False, maxlen=200):
    ...
    for k in range(t, end):
        ...
        if model_name in ["dkt", "dkt+"]:
            ...
            dcur = dict()
            for key in dforget:
                curd = torch.tensor([[dforget[key][k]]]).long().to(next(model.parameters()).device)
        ...
        if model_name in ["atdkt"]:  ## need change!
            ...
            pred = y[0][-1][cout.item()]
        elif model_name in ["kqn", "saint"]:
            #### 输入有question！
            if qout != None:
                curq = torch.tensor([[qout.item()]]).to(next(model.parameters()).device)
                qin = torch.cat((qin, curq), axis=1)
            curc = torch.tensor([[cout.item()]]).to(next(model.parameters()).device)
            ...
            if atkt_pad == True:
                ...
                pad = torch.tensor([0] * (padlen)).unsqueeze(0).to(next(model.parameters()).device)
                # curc = torch.tensor([[cout.item()]]).to(get_device())
            ...
            curc, curr = torch.tensor([[cout.item()]]).to(next(model.parameters()).device), torch.tensor([[true.item()]]).to(next(model.parameters()).device)
            ...
            curc, curr = torch.tensor([[cout.item()]]).to(next(model.parameters()).device), torch.tensor([[1]]).to(next(model.parameters()).device)
            cin, rin = torch.cat((cin, curc), axis=1), torch.cat((rin, curr), axis=1)
            ...
            if qout != None:
                ...
                qin = torch.tensor([[]]).to(next(model.parameters()).device)
                qinshft = torch.tensor([[]]).to(next(model.parameters()).device)
            curc, curr = torch.tensor([[cout.item()]]).to(next(model.parameters()).device), torch.tensor([[1]]).to(next(model.parameters()).device)
            cinshft, rinshft = torch.cat((cin[:, 1:], curc), axis=1), torch.cat((rin[:, 1:], curr), axis=1)
            dcurinfos = {"qseqs": qin, "cseqs": cin, "rseqs": rin, "shft_qseqs": qinshft, "shft_cseqs": cinshft,
                         "shft_rseqs": rinshft}
            ...
            if itout != None:
                curit = torch.tensor([[itout.item()]]).to(next(model.parameters()).device)
                itin = torch.cat((itin, curit), axis=1)
            curq, curr = torch.tensor([[qout.item()]]).to(next(model.parameters()).device), torch.tensor([[1]]).to(next(model.parameters()).device)
            # curc, curr = torch.tensor([[cout.item()]]).to(get_device()), torch.tensor([[true.item()]]).to(get_device())
            ...
            if sdout != None and qdout != None:
                cursd = torch.tensor([[sdout.item()]]).to(next(model.parameters()).device)
                # sdin = torch.cat((sdin, cursd), axis=1)
                curqd = torch.tensor([[qdout.item()]]).to(next(model.parameters()).device)
                # qdin = torch.cat((qdin, curqd), axis=1)
            curq, curc, curr = torch.tensor([[qout.item()]]).to(next(model.parameters()).device), torch.tensor([[cout.item()]]).to(
                next(model.parameters()).device), torch.tensor([[1]]).to(next(model.parameters()).device)
            # qin, cin, rin = torch.cat((qin, curq), axis=1), torch.cat((cin, curc), axis=1), torch.cat((rin, curr), axis=1)
            qinshft, cinshft, rinshft, sdinshft, qdinshft = torch.cat((qin[:, 1:], curq), axis=1), torch.cat(
                (cin[:, 1:], curc), axis=1), torch.cat((rin[:, 1:], curr), axis=1), torch.cat((sdin[:, 1:], cursd),
                                                                                              axis=1), torch.cat(
                (qdin[:, 1:], curqd), axis=1)
            # qinshft, cinshft, rinshft, sdinshft, qdinshft = qin[:,1:], cin[:,1:], rin, sdin[:,1:], qdin[:,1:]
            # print(f"qin:{qin.shape}, qinshft:{qinshft.shape}")
            y = model(qin.long(), cin.long(), sdin.long(), qdin.long(), rin.long(), qinshft.long(), cinshft.long(),
                      sdinshft.long(), qdinshft.long())
            pred = y[0][-1]
        elif model_name == "gkt":
            curc, curr = torch.tensor([[cout.item()]]).to(next(model.parameters()).device), torch.tensor([[1]]).to(next(model.parameters()).device)
            cin, rin = torch.cat((cin, curc), axis=1), torch.cat((rin, curr), axis=1)
            ...
            curc, curr = torch.tensor([[cout.item()]]).to(next(model.parameters()).device), torch.tensor([[1]]).to(next(model.parameters()).device)
            if tout != None:
                curt = torch.tensor([[tout.item()]]).to(next(model.parameters()).device)
                tin = torch.cat((tin, curt), axis=1)
            else:
                tin = torch.tensor([[]]).to(next(model.parameters()).device)
            if qout != None:
                curq = torch.tensor([[qout.item()]]).to(next(model.parameters()).device)
                qin = torch.cat((qin, curq), axis=1)
            curc, curr = torch.tensor([[cout.item()]]).to(next(model.parameters()).device), torch.tensor([[1]]).to(next(model.parameters()).device)
        ...
        cpred = torch.tensor([[predl]]).to(next(model.parameters()).device)
        ...
        fout.write("\t".join(
            [str(idx), str(uid), str(k), str(qidx), str(is_repeat[t:end]), str(len(clist)), str(clist), str(rlist),
             str(cout.item()), str(true.item()), str(pred.item()), str(predl)]) + "\n")
...
def prepare_data(model_name, is_repeat, qidx, dcur, curdforget, dtotal, dforget, t, end, maxlen=200):
    ...
    for k in range(t, end):
        ...
        curc, curr = torch.tensor([[curc.item()]]).to(next(model.parameters()).device), torch.tensor([[curr.item()]]).to(next(model.parameters()).device)
        ...
        if cq.shape[0] > 0:
            curq = cq.long()[k]
            curq = torch.tensor([[curq.item()]]).to(next(model.parameters()).device)
        ...
        if ct.shape[0] > 0:
            curt = ct.long()[k]
            curt = torch.tensor([[curt.item()]]).to(next(model.parameters()).device)
        ...
        if model_name == "lpkt":
            if cit.shape[0] > 0:
                curit = cit.long()[k]
                curit = torch.tensor([[curit.item()]]).to(next(model.parameters()).device)
            ...
            cursd = csd.long()[k]
            cursd = torch.tensor([[cursd.item()]]).to(next(model.parameters()).device)
            curqd = cqd.long()[k]
            curqd = torch.tensor([[curqd.item()]]).to(next(model.parameters()).device)
    ...
    if model_name == "lpkt":
        ...
        finalsds, finalsdshfts, finalqds, finalqdshfts = torch.tensor([]), torch.tensor([]), torch.tensor(
            []), torch.tensor([])
...
def predict_each_group2(dtotal, dcur, dforget, curdforget, is_repeat, qidx, uid, idx, model_name, model, t, end, fout,
                        atkt_pad=False, maxlen=200):
    ...
    # 不用预测结果
    if model_name == "lpkt":
        qidxs, finalqs, finalcs, finalrs, finalts, finalits, finalqshfts, finalcshfts, finalrshfts, finaltshfts, finalitshfts, finald, finaldshft = prepare_data(
            model_name, is_repeat, qidx, dcur, curdforget, dtotal, dforget, t, end)
    elif model_name == "dimkt":
        qidxs, finalqs, finalcs, finalrs, finalts, finalqshfts, finalcshfts, finalrshfts, finaltshfts, finalsds, finalsdshfts, finalqds, finalqdshfts, finald, finaldshft = prepare_data(
            model_name, is_repeat, qidx, dcur, curdforget, dtotal, dforget, t, end)
    else:
        qidxs, finalqs, finalcs, finalrs, finalts, finalqshfts, finalcshfts, finalrshfts, finaltshfts, finald, finaldshft = prepare_data(
            model_name, is_repeat, qidx, dcur, curdforget, dtotal, dforget, t, end)
    bidx, bz = 0, 128
    while bidx < finalcs.shape[0]:
        ...
        if model_name in ["atdkt"]:
            ...
            y = y[:, 1:]
        elif model_name in ["akt", "extrakt", "folibikt", "cakt", "fluckt", "lefokt_akt", "akt_vector", "akt_norasch",
                            "akt_mono", "akt_attn", "aktattn_pos", "aktmono_pos", "akt_raschx", "akt_raschy",
                            "aktvec_raschx"]:
            ...
            if atkt_pad == True:
                ...
                pad = torch.tensor([0] * padlen).unsqueeze(0).expand(curc.shape[0], padlen).to(next(model.parameters()).device)
            ...
            ccsd = cursd
            ccqd = curqd
            y = model(ccq.long(), ccc.long(), ccsd.long(), ccqd.long(), ccr.long(), curqshft.long(), curcshft.long(),
                      cursdshft.long(), curqdshft.long())
        elif model_name in ["bakt_time"]:
            dcurinfos = {"qseqs": curq, "cseqs": curc, "rseqs": curr,
        ...
        for i in range(0, curc.shape[0]):
            ...
            fout.write("\t".join(
                [str(idx), str(uid), str(bidx + i), str(qidx), str(len(clist)), str(clist), str(rlist), str(cshftlist),
                 str(rshftlist), str(true[i]), str(pred[i]), str(predl)]) + "\n")
