#!/bin/bash

# 创建日志目录
mkdir -p saved_model/bs/logs

# 定义运行实验的函数
run_dataset_experiments() {
    local DATASET=$1
    local GPU_ID=$2
    
    echo "Starting VCRKT Visual experiments for Dataset: $DATASET on GPU: $GPU_ID"
    
    # 构造相对保存路径
    REL_SAVE_DIR="saved_model/bs/visual"
    mkdir -p "$REL_SAVE_DIR"
    
    # 构造日志文件名
    LOG_FILE="saved_model/bs/logs/visual_${DATASET}.log"
    
    echo "[GPU $GPU_ID] Running: Dataset=$DATASET Type=Visual"
    echo "[$(date)] Starting Training..." > "$LOG_FILE"
    
    (
        # 切换到脚本目录执行 (保持相对路径一致性)
        cd scripts_training2testing/examples && \
        # 1. 训练
        python wandb_vcrkt_train.py \
        --dataset_name "$DATASET" \
        --model_name "vcrkt" \
        --question_rep_type "visual" \
        --save_dir "../../$REL_SAVE_DIR" \
        --dim_qc 200 \
        --d_question 1024 \
        --gpu_id "$GPU_ID" \
        --num_epochs 200 \
        --use_wandb 0 \
            >> "../../$LOG_FILE" 2>&1
        
        train_exit_code=$?
        
        if [ $train_exit_code -eq 0 ]; then
            echo "[$(date)] Training process finished." >> "../../$LOG_FILE"
        else
            echo "[$(date)] Training Failed with code $train_exit_code" >> "../../$LOG_FILE"
        fi
    )
    
    echo "[GPU $GPU_ID] Finished: Dataset=$DATASET Type=Visual"
}

# 并行运行三个数据集 (GPU分配: 0, 1, 2)
# 注意：数据集名称必须与 data_config.json 中的键完全匹配 (区分大小写)

# GPU 0: XES3G5M (注意全大写) -> 改为 GPU 1
run_dataset_experiments "XES3G5M" 1 &

# GPU 1: DBE_KT22 (注意全大写) -> 改为 GPU 2
run_dataset_experiments "DBE_KT22" 2 &

# GPU 2: nips_task34 (注意小写) -> 改为 GPU 3
run_dataset_experiments "nips_task34" 3 &

# 等待所有后台任务完成
echo "All Visual experiments launched in parallel. Waiting for completion..."
wait
echo "All Visual experiments finished."
